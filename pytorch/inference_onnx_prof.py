...
def onnx_audio_tagging(args, ort_session):
    ...
    return clipwise_output, labels, time_elapse
...
if __name__ == '__main__':
    ...
    if args.mode == 'audio_tagging':
        average_time = 0
        onnx.checker.check_model(onnx.load(args.onnx_path))
        ort_session = onnxruntime.InferenceSession(args.onnx_path, providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
        for i in range(10):
            _, _, eval_time = onnx_audio_tagging(args, ort_session)
            average_time += eval_time

        print(f"Average Time: {average_time/10:.6f}")
